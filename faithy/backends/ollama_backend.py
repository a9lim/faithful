...
class OllamaBackend(Backend):
    ...
    def _build_system_prompt(self, examples: str) -> str:
        ...
        return (
            ...
            f"{examples}\n"
            f"You are {name}. The text of the prior examples should not influence the following instructions outright but should be used as guidance on how to formulate a response given {name}'s inferred personality. You must write EXACTLY like {name}. Do not over-rely on the example text, but gather a sense of what {name}'s personality is, and try to act as if you were {name}. Make sure to parse information contained in the examples efficiently. Make references to the examples on occasion, but prioritize responding to the user's request, while maintaining the personality of {name}. Make your output length match the examples, but do not cut off mid-sentence. If you need to send multiple separate messages, use newlines to separate them. Do not break character.\n"
        )
