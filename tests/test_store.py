...
class TestMessageStore:
    ...
    def test_load_from_txt(self, tmp_path: Path):
        (tmp_path / "msgs.txt").write_text("hello\nworld\n")
        store = MessageStore(_make_config(tmp_path))
    ...
    def test_remove_message(self, tmp_path: Path):
        (tmp_path / "msgs.txt").write_text("a\nb\nc\n")
        store = MessageStore(_make_config(tmp_path))
    ...
    def test_get_all_text(self, tmp_path: Path):
        (tmp_path / "msgs.txt").write_text("one\ntwo\n")
        store = MessageStore(_make_config(tmp_path))
    ...
    def test_reload(self, tmp_path: Path):
        store = MessageStore(_make_config(tmp_path))
        assert store.count == 0
        (tmp_path / "msgs.txt").write_text("added later\n")
        store.reload()
        assert store.count == 1

    def test_multiple_files(self, tmp_path: Path):
        (tmp_path / "a.txt").write_text("from_a\n")
        (tmp_path / "b.txt").write_text("from_b\n")
        store = MessageStore(_make_config(tmp_path))
        assert store.count == 2
